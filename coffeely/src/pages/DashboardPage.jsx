 ...
 * Layout:
 *   ┌─────────────────────────────────────────┐
 *   │  Banner (si hay captura pendiente)      │
 *   │  Saludo + fecha + botón registrar       │
 *   │  [Métrica][Métrica][Métrica][Métrica]   │ ← 4 cols xl, 2 md, 1 sm
 *   │  [  CashFlowChart (2/3)  ][Semáforo]   │ ← 3 cols lg
 *   └─────────────────────────────────────────┘
  ...
  return (
    <div role="alert" className={`rounded-2xl border ${bgCls} px-5 py-4 flex items-start gap-4`}>
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button type="button" onClick={onDismiss} aria-label="Cerrar recordatorio"
    ...
    <div className="min-h-screen bg-bg-light flex flex-col font-sans text-text-main">
      ...
      <main
        ...
        className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full flex flex-col gap-8
          pb-28"  /* padding-bottom extra para que el chatbot flotante no tape contenido */
      >

        {/* Banner pendiente */}
        {showBanner && (
          <PendingBanner {...pendingData} onDismiss={() => setBannerDismissed(true)} />
        )}

        {/* ── Saludo + acceso rápido ── */}
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
          <div>
            <h1 className="text-2xl font-bold text-dark-olive tracking-tight">
              {greeting(name)}
            </h1>
            <p className="text-sm text-text-muted capitalize mt-0.5">{today}</p>
          </div>
          <div className="flex items-center gap-3">
            <p className="metric-label hidden sm:block">{t('dashboard.subtitle')}</p>
            <Link
